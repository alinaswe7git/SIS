...
class studies {
	...
	public function manageStudies($item=null) {
		...
		if(!empty($item)){
			$sql = "SELECT * FROM `study`,`schools` WHERE `study`.ParentID = `schools`.ID AND `study`.ID = $item ORDER BY `study`.schools,`study`.Name";
		}else{
			$sql = "SELECT `study`.ID, `study`.Name, `schools`.name, `schools`.id , `study`.Delivery,`study`.ShortName  FROM `study`,`schools` WHERE `study`.ParentID = `schools`.ID ORDER BY `schools`.name,`study`.Name";
		}
		...
		while ($row = $run->fetch_row()) {
			...
			echo '<tr ' . $bgc . '>
			<td><b><a href="' . $this->core->conf['conf']['path'] . '/studies/show/' . $row[0] . '"> ' . $row[1] . ' ('.$row[5] .')</a></b></td>' .
