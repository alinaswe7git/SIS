...
class accounting {
	...
	public function balancesAccounting($item, $listType = "list") {
		...
		echo'<h1>NOW FILTERED TO ONLY CURRENTLY REGISTERED</h1>';
		...
		$sql = "SELECT  `FirstName`,`Surname`,`ID`,`StudyType`,`Status`,`balances`.`StudentID`, `Amount`  FROM `basic-information`, `balances` 
			...
			AND `basic-information`.ID IN (SELECT StudentID FROM `course-electives` WHERE `PeriodId` = '57' AND `Approved` = 1)
			AND `balances`.`Amount` > $amount 
			AND `balances`.`Amount` != '1000000000'
	...
	private function showInfoList($sql, $count, $total, $intake, $print) {
		...
		while ($row = $run->fetch_row()) {
			...
			if(isset($row[7]) && $print == FALSE){
				...
				$extra = "BALANCE: ";
				$txt = '<td ><span class="glyphicon glyphicon-user" aria-hidden="true" style="color: red"></span></td>
				<td colspan="2"><a href="' . $this->core->conf['conf']['path'] . '/information/show/' . $id . '"><b> '.$firstname.' '.$surname.'</b></a></td>';
			}else{
				$date = NULL;
				$txt = '<td><span class="glyphicon glyphicon-user" aria-hidden="true" style="color: red"></span></td><td>'.$count.'</td>
