...
class cards{
	...
	function frontCards($item){ 
		...
		while ($row = $run->fetch_assoc()) {
			...
			$middlename = ucfirst($row['Surname']);
			...
			$mode = $row['StudyType'];
			$sstatus = $row['Status']; 
			if($sstatus == 'New' || $sstatus == 'Requesting' || $sstatus == 'Approved'|| $sstatus == 'Inactive'){
				...
				echo'<div style="padding: 15px; height: 250px; break-inside: avoid; text-align:center; width: 350pt;">
					<div class="university" style="font-size: 19pt; font-weight:bold; color: #FFF;  text-align: center; font-family: arial; background-color: '.$color.'; padding: 2pt;"> 
						NIPA
					</div>
					<div class="subtitle" style="text-align: left;">
						<span style="font-size: 14pt; font-weight:bold; text-align: left; font-family: arial; padding: 2pt;"> STUDENT ID  |</span>   
					...
					<div style="width: 105pt; border: 2px solid #000; float: left; margin-top: 15px; ">'.$profile.'</div>
						...
						<div style="width: 100%; text-align: center;  font-family: arial; padding-top: 0px;"><img width="82" src="../../templates/loanpro/images/header.png"></div>
						...
						<div class="studentid" style=" font-family: arial;  float:left; width: 85pt; text-align: left;"> STUDENT ID:  </div> <div class="studentid" style=" font-family: arial; font-weight: bold; float:left;">  '.$uid.' </div><br>
				...
				<div style="page-break-before: always; text-align: center; width: 350pt;">
					...
					<p style="font-size: 14pt;">
						<b>THIS CARD REMAINS THE PROPERTY OF<br> '.$this->core->conf['conf']['institutionName'].'</b> 
						<br> IF FOUND PLEASE RETURN TO THE INSTITUTION OR NEAREST POLICE STATION.
					</p>
					NRC: '.$nrc.'
					<br><img src="'.$this->core->conf['conf']['path'].'/barcodes/src/test.php?id='.$uid.'"> <br> '.$uid.'
				</div>';
			}else if($sstatus == 'Employed'){
				
				$sqlx = "SELECT * FROM `staff` WHERE `EmployeeNo` = '$item'";
				$runx = $this->core->database->doSelectQuery($sqlx);
				$rowx = $runx->fetch_assoc();
				
				$position = $rowx['JobTitle'];
				$date = $rowx['EndDate'];
				...
				echo'<div style="padding: 15px; height: 250px; break-inside: avoid; text-align:center; width: 350pt;">
					...
					<div class="subtitle" style="text-align: left;">
						<span style="font-size: 14pt; font-weight:bold; text-align: left; font-family: arial; padding: 2pt;"> STAFF IDENTITY CARD  |</span>   
						<span style=" font-size: 14pt; font-weight:bold; font-family: arial;color: #dc7400;"> MAN No. :'.$uid.' </span>
					</div>
					...
					<div style="width: 105pt; border: 2px solid #000; float: left; margin-top: 15px; ">'.$profile.'</div>
						...
						<div style="width: 100%; text-align: center;  font-family: arial; padding-top: 0px;"><img width="82" src="../../templates/loanpro/images/header.png"></div>
						<div class="studentname" style="padding-top: 0pt; padding-bottom: 10pt; font-size: 15pt; text-align: left;  font-family: arial; text-align:center;">' . $firstname . ' ' . $middlename . '<br> <span style="font-size: 18pt; font-weight:bold;  ">' . $surname . '</span> </div>
						<div class="studentid" style=" font-family: arial;  float:left; width: 85pt; text-align: left;"> Position:  </div> <div class="studentid" style=" font-family: arial; font-weight: bold; float:left;">  '.$position.' </div><br>
						<div class="studentid" style=" font-family: arial;  float:left; width: 85pt; text-align: left;"> VALID UNTIL:  </div> <div class="studentid" style=" font-family: arial; font-weight: bold;  float:left;"> '.$date.' </div>
					</div>
				</div>
				...
				<div style="page-break-before: always; text-align: center; width: 350pt;">
					...
					NRC: '.$nrc.'
					<br><img src="../../barcodes/src/test.php?id='.$uid.'"> <br> '.$uid.'
				</div>';
			}else{
				echo 'Please check and update the user Status on the system.'; 
			}
		}
	}
	...
	function backCards($item){ 
		...
		while ($row = $run->fetch_row()) {
			...
			<p style="font-size: 14pt;"><b>THIS CARD REMAINS THE PROPERTY OF<br> KWAME NKRUMAH UNIVERSITY</b> <br> IF FOUND PLEASE RETURN TO THE UNIVERSITY OR NEAREST POLICE STATION. <br> <br>www.mu.ac.zm</p>
			<br><img src="https://edurole.mu.ac.zm/barcodes/src/test.php?id='.$uid.'"> <br> '.$uid.'
			</div>';
			echo'</div>';
		}
	}
