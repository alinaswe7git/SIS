...
class users {
	...
	function saveUsers() {
		...
		$sql = "INSERT INTO `basic-information` (`FirstName`, `MiddleName`, `Surname`, `Sex`, `ID`, `GovernmentID`, `DateOfBirth`, `PlaceOfBirth`, `Nationality`, `StreetName`, `PostalCode`, `Town`, `Country`, `HomePhone`, `MobilePhone`, `Disability`, `DissabilityType`, `PrivateEmail`, `MaritalStatus`, `StudyType`, `Status`) 
				VALUES ('$firstname', '$middlename', '$surname', '$sex', NULL, '$id', '$dob', '$pob', '$nationality', '$streetname', '$postalcode', '$town', '$country', '$homephone', '$celphone', '$dissability', '$dissabilitytype', '$email', '$mstatus', '$studytype', 'Employed');";
		...
		if ($this->core->database->doInsertQuery($sql)) {
			...
			while ($fetch = $run->fetch_assoc()) {
				...
				$sql = "INSERT INTO `access` (`ID`, `Username`, `RoleID`, `Password`) 
						VALUES ('".$fetch['ID']."', '$username', '$roleid', '$passenc');";
	...
	function manageUsers() {
		...
		if($this->core->pager == FALSE){
			...
			echo '<table class="table" height="" border="0" cellpadding="3" cellspacing="0">
		...
		$sql = "SELECT *, `basic-information`.ID as UID
		...
		while ($row = $run->fetch_assoc()) {
			...
			echo '<tr>
				...
				<td style=""> <img src="' . $this->core->fullTemplatePath . '/designerpack/person-circle-outline.svg" alt="icon" width="20" height="20"> <a href="' . $this->core->conf['conf']['path'] . '/information/show/' . $uid . '" style="'.$style.'"><b>' . $firstname . ' ' . $middlename . ' ' . $surname . '</b></a></td>
				<td style=""><i>' . $role . '</i></td>
				<td style=""><b>' .$username .'</b></td>
				<td style="">' . $status . '</td>
				<td style=""><a href="' . $this->core->conf['conf']['path'] . '/information/edit/' . $uid . '"><img src="' . $this->core->fullTemplatePath . '/designerpack/pencil-outline.svg" alt="icon" width="15" height="15"> edit</a>  <a href="' . $this->core->conf['conf']['path'] . '/users/delete/' . $uid . '" onclick="return confirm(\'Are you sure?\')"><img src="' . $this->core->fullTemplatePath . '/designerpack/close-circle.svg" alt="icon" width="15" height="15"> delete</a></td>
			</tr>';
		}
	...
	function studentsUsers() {
		...
		echo '<div class="resultrow">
				...
				<div style="width: 100px; float:left;"><a href="' . $this->core->conf['conf']['path'] . '/information/edit/' . $uid . '"><img src="' . $this->core->fullTemplatePath . '/designerpack/pencil-outline.svg"> edit</a>  <a href="' . $this->core->conf['conf']['path'] . '/users/delete/' . $uid . '" onclick="return confirm(\'Are you sure?\')"><img src="' . $this->core->fullTemplatePath . '/designerpack/close-circle-outline.svg"> delete</a></div>
		  	</div>';
		}
