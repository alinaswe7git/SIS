...
class billing {
	...
	public function personalBilling($item) {
		...
		foreach($payment as $trans){
			...
			if (substr($iid, 0,2) == "CN") { $type = "CREDIT NOTE"; continue; }
			if (substr($iid, 0,2) == "IN") { $type = "BILLING"; }
			if (substr($iid, 0,2) == "UC") { $type = "PAYMENT"; continue; }
			...
			echo '<tr ' . $color . '>
			<td style="border: 1px solid #ccc;"><b><a href="' . $this->core->conf['conf']['path'] . '/billing/view/' . $or . '"> ' . $iid . '</a></b></td>

			<td>' . $date . '</td>
			<td style="border: 1px solid #ccc;"><b>'. $type .'</b></td>
			<td style="border: 1px solid #ccc;"><i>'.$description.'</i></td>
			<td style="text-align: right; border: 1px solid #ccc;">' . $debit . ' ZMW</td>';
		...
		<td colspan="4"><b> TOTAL BILLED</b></td>
		<td style="text-align: right;"> <b>'.$balance .' ZMW</b> </td>
		</tr>';
