			...
			$(function() {
					...
					if (e.keyCode == 13) { 
						...
						$.ajax({
							...
							success: function(data){
								$("#messages"+escapeSelector(name)).append("<div class=\"row msg_container base_sent\"><div class=\"gavatar\"><img src=\"'.$savatar.'\" class=\" img-responsive \"></div><div class=\"col-md-10 col-xs-10\"><div class=\"messages msg_sent\"><p>" + data.message + "</p><time datetime=\"" + data.date + "\">You - " + data.date + "</time></div></div></div>");
								$("#input"+escapeSelector(name)).val("");
								$("#messages"+escapeSelector(name)).animate({ scrollTop: $("#messages"+escapeSelector(name)).prop("scrollHeight")}, 1000);
							}
						});
    					}
 				});
				...
				$(document).on("click", "[id^=userid]", function (e) {
					...
					$.ajax({
						...
						dataType: "json",
						success: function(data){
							$("#messages"+escapeSelector(name)).append(data.messages);
							$("#messages"+escapeSelector(name)).animate({ scrollTop: $("#messages"+escapeSelector(name)).prop("scrollHeight")}, 1000);
						}
					});
...
/*
			...
			echo'	window.setInterval(function(){
					...
					$("[id^=userid]").each(function(){
						var name = $(this).attr("id").substring(6); 

						$.ajax({
							type: "GET",
							url: "'.$this->core->conf['conf']['path'].'/api/chat/check",
							data: { 	"last" : num, "sender" : name },
								dataType: "json",
								success: function(data){
									$("#messages"+escapeSelector(name)).append(data.messages);
									$("#messages"+escapeSelector(name)).animate({ scrollTop: $("#messages"+name).prop("scrollHeight")}, 1000);
								}
							});
...
*/
		echo'		$(document).on("click", "#chatopen", function (e) {
					$("#chatwindow").show();
				});
		...
		// UNREAD MESSAGES
		$sqlc = "SELECT COUNT(`chat`.ID) as Count, `SenderID`, `FirstName`, `Surname`, `SenderID` as UID
			FROM `chat`, `basic-information` 
			WHERE `RecipientID` LIKE '$sender'
		...
		foreach($names as $id=>$name){
			...
			echo '<div class="user" id="userid'.$uid.'" style="background-color: #ffe2ad;">
				<span  class="glyphicon glyphicon-user icon_user"></span><a href="#"> <b>'.$name.' '.$display.'</b></a>
			</div>';
		}
		...
		// ALL CHAT USERS LOGGED IN TODAY
		$sql = "SELECT DISTINCT `user`, `FirstName`, `Surname`, `access`.Username as UID 
			FROM `acl`, `access`
			LEFT JOIN `basic-information` ON `basic-information`.ID = `access`.ID
			WHERE `acl`.user = `access`.Username
			AND `access`.Username != '$sender'
