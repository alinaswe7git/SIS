...
class grades {
	...
	public function courseGrades($item) {
		...
		$sql = "SELECT AcademicYear, Semester, ProgramNo, CourseNo, user, b1.FirstName, b1.Surname, CAMarks, CAMarks, CourseDescription, ExamMarks, TotalMarks, b2.ID, b2.FirstName, b2.Surname  
			FROM `grades`
			LEFT JOIN `basic-information` as b1 ON `grades`.`user` = `b1`.ID
			LEFT JOIN `basic-information` as b2 ON `grades`.`StudentNo` = `b2`.ID
			LEFT JOIN `courses` ON `courses`.Name  = `grades`.CourseNo
			WHERE b2.ID = '$item'
			ORDER BY `grades`.`AcademicYear`, `grades`.`CourseNo`  ASC";
	...
	public function manageGrades($sql, $ca) {
		...
		if($this->core->role == 1000 && empty($sql)){
			$sql = "SELECT AcademicYear, Semester, ProgramNo, CourseNo, user, FirstName, Surname, count(StudentNo), userdate , `CourseDescription`
			FROM `grades`
		...
		while ($fetch = $run->fetch_assoc()) {
			...
			if($init == FALSE){
				...
				echo '<td width="100px"><b>Course name:</b></td>';
			...
			echo  '<td width="70px"><b>'.$coursedesc.'</b></td>';
	...
	function listGrades() {
		...
		include $this->core->conf['conf']['formPath'] . "atables.form.php";

		echo'<table id="resList" class="table table-bordered table-striped table-hover">
			<thead>
			<tr class="heading">
				<td>#</td>
				<td>Course Code</td>
