...
echo '<label style="margin-left: 40px; ">SELECT ALL<input type="checkbox" name="select-all" id="select-all" /></label>';
echo '<script>
		...
		while ($fetch = $run->fetch_assoc()) {
			...
			echo '<tr>
        		<td><a href="' . $this->core->conf['conf']['path'] . '/roles/permissions/' . $fetch['FunctionID'] . '"><b>'.$class.' / '.$function.'</b></a> (' . $title . ')</b></td>
                    ...
                    <td>
                    <a href="' . $this->core->conf['conf']['path'] . '/roles/permissions/delete/' . $fetch['FunctionID'] . '/'.$item.'" onclick="return confirm(\'Are you sure?\')"> <img src="' . $this->core->fullTemplatePath . '/designerpack/close-circle.svg" alt="icon" width="15" height="15"> Delete </a>
                    </td>
                    </tr>';
		}
	...
	function manageRoles() {
		...
		while ($fetch = $run->fetch_row()) {
			...
			<td>' . $fetch[4] . ' ' . $fetch[5] . '</td>
                    <td>
                    <a href="' . $this->core->conf['conf']['path'] . '/roles/edit/' . $fetch[0] . '"> <img src="' . $this->core->fullTemplatePath . '/designerpack/pencil-outline.svg" alt="icon" width="15" height="15"> edit</a>
                    <a href="' . $this->core->conf['conf']['path'] . '/roles/delete/' . $fetch[0] . '" onclick="return confirm(\'Are you sure?\')"> <img src="' . $this->core->fullTemplatePath . '/designerpack/close-circle.svg" alt="icon" width="15" height="15"> delete </a>
                    </td>
                    </tr>';
		}
