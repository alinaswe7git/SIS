...
class home
{
	...
	public function showHome($item)
	{
		...
		//get the user id (student number)
		...
		// $sql = "SELECT * FROM periods p WHERE p.ID = '" . $period . "' ";
		// $run = $this->core->database->doSelectQuery($sql);

		// while ($row = $run->fetch_row()) {

		// 	$courseRegStartDate = $row[7];
		// }
		// //.................................................................................................

		// $period  = $this->core->getPeriod();
		// $sql = "SELECT * FROM `course-electives` ce WHERE ce.StudentID = '" . $idnumber . "' and ce.PeriodID = '" . $period . "' ";
		// $run = $this->core->database->doSelectQuery($sql);
		// $this->infoSheet();
		// if ($run->num_rows >= 1) {
		// 	// echo "<script>
		...
		// 	// </script>";
		// 	$items->overviewItem('memo', "You are registered for this academic year");
		// } else {
		// 	echo "<script>
		// 	alertify.alert('You have not yet registered!');
		// 	</script>";
		// }
		...
		// $balance = $payments->getBalance($idnumber);
		// //echo $balance . "bbbbb";


		// $items->overviewItem('news', "News and updates");
		// $items->overviewItem('memo', "Memorandums relevant to you");
		// $items->overviewItem('memo', "You have no outstanding bills fo the current semester, your balance is K");
		// $items->overviewItem('memo', "Registration Start Date : " . $courseRegStartDate);
	...
	public function details($items)
	{
		...
		if ($status != "Employed") {
			...
			if ($run->num_rows >= 1) {
				...
				$sql = "SELECT amount FROM acct_invoice ai where studentNo = '" . $idnumber . "' and description = 'Tuition Fees' and academicYear = (SELECT year FROM periods p where ID = '" . $period . "') and Semester = (SELECT `Semester` FROM periods WHERE ID = '" . $period . "');";
				$run = $this->core->database->doSelectQuery($sql);
				...
				if( $balance > 0 ){
					$items->overviewItem('memo', "You owe the institution K" . $balance);
				}
				elseif($balance < 0){
					$items->overviewItem('memo', "You have no outstanding bills for this semester, your Account balance is K" . $balance);
				}
	...
	private function globalStatistics()
	{
		...
		(SELECT count(`basic-information`.ID) FROM `access`, `basic-information` WHERE `access`.ID = `basic-information`.ID AND `Status` = 'Employed') AS users";
		...
		echo '<div style="width: 100%">
        	...
        	<div class="statistics">' . $this->core->translate("SMS available") . ': <div class="statistic">50,000';
		//$sms->unitsSms();
		echo '</div></div>
        	</div>';
	}
		...
		while ($row = $run->fetch_row()) {
			...
			while ($row = $runx->fetch_assoc()) {
				...
				echo "    var notification = new Notification('$name', {
				  icon: '../../templates/edurole/images/apple-touch-icon-144x144-precomposed.png',
				  body: '$content',
				});
	...
	private function easyMenu()
	{
		if ($this->core->role > 10) {
			echo '<div style="float:left; margin-bottom: 5px; width: 100%">
                 	<div class="easymen"><a href="http://library.mu.ac.zm/"><img src="' . $this->core->fullTemplatePath . '/images/books.png"> <br>  Library</a></div>
