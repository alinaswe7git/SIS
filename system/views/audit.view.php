...
class audit{
	...
	public function transactionsAudit($item) {
		...
		if(empty($item)){
			$sql = "SELECT * FROM `audit`, `basic-information` WHERE `AuditType` = 'payments' AND `basic-information`.ID = `UserID` AND DATE(`DateTime`) = CURRENT_DATE";
		}  else {
			$sql = "SELECT * FROM `audit`, `basic-information` WHERE `AuditType` = 'payments' AND `basic-information`.ID = `UserID` AND `DateTime` = '$item'";
		}
