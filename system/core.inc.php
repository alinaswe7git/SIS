...
class eduroleCore {
        ...
        public function __construct($conf, $initialize = TRUE) {
                ...
                $this->logEvent("Initializing AviPlat core", "3");
	...
	public function getPeriod(){
        ...
        public function audit($type, $reference,  $user,  $description) {
                ...
                $sql = "INSERT INTO `audit` (`ID`, `URL`, `UserID`, `Action`, `ReferenceID`, `AuditData`, `DateTime`, `AuditType`, `IP`)
                 VALUES (NULL, '$url', '$admin', \"$description\", '$user', '$reference', NOW(), \"$type\", \"$ip\");";
	...
	public function getCurrentPeriod(){
		...
		if($run->num_rows == 0){
			$sql = "INSERT INTO `periods` (`ID`, `PeriodStartDate`, `PeriodEndDate`, `Year`, `Semester`, `Name`, `Delivery`, `CourseRegStartDate`, `CourseRegEndDate`, `Weeks`, `Intake`) VALUES (NULL, '0000-00-00', '0000-00-00', '$academicyear', '$semester_int', '$semester', '', '0000-00-00', '0000-00-00', '0', '$date_year');";
        ...
        public function redirectsub($page, $action, $item = NULL, $sub) {
                $base = $this->conf['conf']['path'];

                $page = $page == NULL ? "" : $page . "/";
                $item = $item == NULL ? "" : "/".$item;

                $url = $base ."/". $page . $action . $item . "?period=".$sub;
